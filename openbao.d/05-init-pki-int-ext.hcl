initialize "pki-int" {
  ...
  request "import_int" {
    operation = "create"
    path = "pki-int/issuers/import/bundle"
    data = {
      pem_bundle = {
        eval_source = "response"
        eval_type = "string"
  ...
  request "cluster" {
    operation = "create"
    path = "pki-int/config/cluster"
    data = {
      path = "http://bao.local:8200/v1/pki-int"
    }
  }
  request "acme" {
    operation = "create"
    path = "pki-int/config/acme"
    data = {
      enabled = true
    }
  }
